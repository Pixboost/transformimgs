...
func (p *ImageMagick) isIllustration(src *img.Image, info *img.Info) (bool, error) {
	...
	for colorIdx, currColor = range colors {
		...
		if isBackground {
			fmt.Printf("Compare colors %s and %s\n", currColor.GetColorAsString(), lastBackgroundColor.GetColorAsString())
			distance :=
				math.Pow(currColor.GetRed()-lastBackgroundColor.GetRed(), 2) +
					math.Pow(currColor.GetGreen()-lastBackgroundColor.GetGreen(), 2) +
					math.Pow(currColor.GetBlue()-lastBackgroundColor.GetBlue(), 2)
			if distance < 0.1 {
				lastBackgroundColor = currColor
				pixelsInBackground += count
				colorsInBackground++
	...
	colorsCntIn50Pct := uint(colorIdx + 1)
	colorsCntIn50Pct -= colorsInBackground
